...
public class LanguageRuntimeService {
    ...
    /**
     * Detect the user's login shell on macOS/Linux.
     * Returns "zsh" or "bash". Falls back to "bash" if undetectable.
     */
     ...
     * macOS/Linux: use detected shell (zsh or bash) with login mode
     */
    ...
    /**
     * Build a shell preamble that sources common SDK version managers
     * (nvm, sdkman, gvm, etc.) so their shell functions are available.
     * Sources shell-specific rc files and SDK init scripts.
     */
    public static String buildShellPreamble(String shell) {
        ...
        if ("zsh".equals(shell)) {
            sb.append(" && { [ -f \"").append(home).append("/.zshrc\" ] && source \"").append(home).append("/.zshrc\"; } || true");
            sb.append(" && { [ -f \"").append(home).append("/.zprofile\" ] && source \"").append(home).append("/.zprofile\"; } || true");
        } else {
            sb.append(" && { [ -f \"").append(home).append("/.bash_profile\" ] && source \"").append(home).append("/.bash_profile\"; } || true");
            sb.append(" && { [ -f \"").append(home).append("/.bashrc\" ] && source \"").append(home).append("/.bashrc\"; } || true");
        }
        ...
        for (String script : initScripts) {
            sb.append(" && { [ -f \"").append(script).append("\" ] && source \"").append(script).append("\"; } || true");
        }
        return sb.toString();
    }
    ...
    /**
     * Append common macOS/Homebrew/SDK paths to the process PATH so that
     * tools installed via Homebrew, SDKMAN, nvm, gvm, etc. are discoverable
     * even when running in a non-interactive login shell.
     */
