...
public class SettingsController {
    ...
    public String settingsPage(Model model) {
        model.addAttribute("maxConcurrent", settingsService.getInt(SystemSettingsService.KEY_MAX_CONCURRENT, 20));
        model.addAttribute("logRetentionDays", settingsService.getInt(SystemSettingsService.KEY_LOG_RETENTION, 7));
        model.addAttribute("historyRetentionDays", settingsService.getInt(SystemSettingsService.KEY_HISTORY_RETENTION, 90));
        model.addAttribute("buildServerHost", settingsService.get(SystemSettingsService.KEY_BUILD_SERVER_HOST));
        model.addAttribute("buildServerPort", settingsService.getInt(SystemSettingsService.KEY_BUILD_SERVER_PORT, 22));
        model.addAttribute("buildServerUser", settingsService.get(SystemSettingsService.KEY_BUILD_SERVER_USER));
        model.addAttribute("buildServerAuthEnv", settingsService.get(SystemSettingsService.KEY_BUILD_SERVER_AUTH_ENV));
    ...
    public String saveRetention(@RequestParam int logRetentionDays,
                                @RequestParam int historyRetentionDays) {
    ...
    public String saveBuildServer(@RequestParam String host,
    ...
    public String addEnvVar(@RequestParam String varKey, @RequestParam(required = false) String description) {
    ...
    public List<String> listSystemEnvVars() {
        ...
        if (cachedEnvVars == null || (now - cacheTime) > CACHE_TTL_MS) {
            cachedEnvVars = System.getenv().keySet().stream()
                    .sorted()
